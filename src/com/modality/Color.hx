...
class Color
{
  ...
  {
    return (Math.floor(rgb.r * 255) << 16) | (Math.floor(rgb.g * 255) << 8) | Math.floor(rgb.b * 255);
  }

  public static function hsv2int(_h:Float, _s:Float, _v:Float):Int
  {
    return toInt(hsv2rgb({h:_h, s:_s, v:_v}));
  }
  
  public static function hsl2int(_h:Float, _s:Float, _l:Float):Int
  {
    return toInt(hsl2rgb({h:_h, s:_s, l:_l}));
  }
